...
resource "aws_launch_template" "eks_worker_template" {
  ...
  user_data                   = base64encode(data.template_file.eks_worker_userdata[each.key].rendered)
...
resource "aws_autoscaling_group" "eks_worker_asg" {
  ...
  mixed_instances_policy {
    launch_template {
      launch_template_specification {
        launch_template_id = aws_launch_template.eks_worker_template[each.key].id
        version            = "$Latest"
      }
      override {
        instance_type = each.value.instance_type
  ...
  lifecycle {
    ...
    ignore_changes        = [desired_capacity, desired_capacity_type, target_group_arns]
  }
}