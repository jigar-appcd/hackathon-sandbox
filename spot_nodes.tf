data "template_file" "eks_worker_userdata_spot" {
  for_each = var.spot_node_groups
...
resource "aws_launch_template" "eks_worker_template_spot" {
  for_each                    = var.spot_node_groups
  name_prefix                 = "${local.resource_name}-${upper(each.key)}"
  image_id                    = each.value.ami_id == null ? var.ami_id : each.value.ami_id
  instance_type               = each.value.instance_type
  key_name                    = var.key_name
  vpc_security_group_ids      = [aws_security_group.eks_worker.id]
  user_data                   = base64encode(data.template_file.eks_worker_userdata_spot[each.key].rendered)
...
resource "random_shuffle" "az_spot" {
  for_each     = var.spot_node_groups
  input        = data.aws_subnets.eks_subnets.ids
  result_count = 1
}

resource "aws_autoscaling_group" "eks_worker_asg_spot" {
  for_each              = var.spot_node_groups
  desired_capacity      = each.value.desired_capacity
  max_size              = each.value.max_size
  min_size              = each.value.min_size
  name                  = "${local.resource_name}-${upper(each.key)}"
  vpc_zone_identifier   = length(each.value.subnets) > 0 ? each.value.subnets : random_shuffle.az[each.key].result
  ...
  mixed_instances_policy {
      ...
      launch_template {
        launch_template_specification {
          launch_template_id = aws_launch_template.eks_worker_template_spot[each.key].id
          version            = "$Latest"
        }
        override {
          instance_type = each.value.instance_type
  ...
  dynamic "tag" {
    for_each = contains(split(",", each.value.node_labels), "") ? [] : split(",", each.value.node_labels)
    content {
  ...
  dynamic "tag" {
    for_each = contains(split(",", each.value.node_taints), "") ? [] : split(",", each.value.node_taints)
    content {
